...
import com.oswald.kv.key.ContactDimension;
import com.oswald.kv.key.DateDimension;
import com.oswald.utils.JdbcInstance;
import com.oswald.utils.JdbcUtil;
import com.oswald.utils.LRUCache;
...
/**
 * DimensionConverter实现类，负责实际的维度转id功能
 * 做内存缓存，LRUCache
 * 分支
 * -- 缓存中有数据 -> 直接返回id
 * -- 缓存中无数据 ->
 ...
 * 查询Mysql
 * 分支
 * -- Mysql中有该数据 -> 直接返回id -> 将本次读取到的id缓存到内存中
